...
router.post(
  ...
  (req, res) => {
    ...
    StudentProfile.findOne({ student: req.body.id }).then(profile => {
      ...
      if (profile) {
        ...
        StudentProfile.findOne({ handle: profileFields.handle }).then(
          profile => {
            if (profile) {
...
router.delete(
  ...
  passport.authenticate('jwt', { session: false }),
  (req, res) => {
    StudentProfile.findOneAndRemove({ student: req.user.id }).then(() => {
